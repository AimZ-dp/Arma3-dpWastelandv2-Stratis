...
private ["_counter","_pos","_markerName","_marker","_hint","_safePos","_boxes", "_nerfBoxes", "_currBox","_boxtype"];
...
for "_i" from 0 to 110 step 25 do
{
	_pos = getMarkerPos format ["Spawn_%1", _i];
    _boxtype = _nerfBoxes select (random (count _nerfBoxes - 1));
    _safePos = [_pos, 25, 50, 1, 0, 60 * (pi / 180), 0] call BIS_fnc_findSafePos;
	_currBox = createVehicle [_boxtype, _safePos,[], 30, "NONE"];
