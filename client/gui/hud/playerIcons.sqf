		...
		{
			{
				...
				drawIcon3D ["", [1,1,1,1], _playerPos, 0, 0, 0, format ["%1 (%2m)", name _x, round (_x distance player)], 0, 0.03, "PuristaLight"];			
			} forEach nameArray;
		...
		{
			{
				...
				_playerPosATL = getPosATL _x;
				_playerPos set [2, (_playerPosATL select 2) + 0.2];
				drawIcon3D ["", [1,1,1,1], _playerPos, 0, 0, 0, name _x, 0, 0.03, "PuristaLight"];			
			} forEach nameArray;
		};
	};
}];
